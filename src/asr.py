...
class LiveASR():
    ...
    def start(self):
        ...
        self.asr_process = threading.Thread(target=LiveASR.asr_process, args=(
            self.model_name, self.asr_input_queue, self.asr_output_queue,))
        self.asr_process.start()
        time.sleep(5)  # start vad after asr model is loaded
        self.vad_process = threading.Thread(target=LiveASR.vad_process, args=(
    ...
    def vad_process(device_name, asr_input_queue):
        ...
        microphones = LiveASR.list_microphones(audio)
        selected_input_device_id = LiveASR.get_input_device_id(
...
if __name__ == "__main__":
    ...
    asr = LiveASR("jonatasgrosman/wav2vec2-large-fr-voxpopuli-french")
    asr.start()
